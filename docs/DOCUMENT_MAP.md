...
progressive_disclosure_surfaces:
  ...
  skill_pack:
    ...
    read_by_default: false
  capability_catalog:
    path: docs/EVOLUTION_MODEL.md
    purpose: canonical state-owner distinction; optional catalog CLI reads only an explicitly supplied index
    read_by_default: false
    read_when:
      - a live issue or PR scopes reusable Lesson or method discovery
