...
| `scripts/asgk_lib/github_workflow.py` | script | sole GitHub workflow implementation: GET-only snapshot capture, issue-backed projection, draft-only handoff/closeout, supplied-snapshot search/trace and common-envelope result boundary | no | GitHub workflow command, work projection, Kanban card draft or closeout-trace review | `lane_06_ci_github` |
