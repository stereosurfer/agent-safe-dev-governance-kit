...
| `docs/control/TARGET_INSTALL_VALIDATION_PLAN.md` | canonical | future target-install validator check categories and output contract | no | target-install validator implementation planning, validation roadmap | `lane_06_ci_github` |
...
| `scripts/asgk.py` | script | minimal ASGK CLI wrapper for doctor/validate/hygiene/negative/status/closeout/pr-body/task-packet/handoff checks | no | local validation, status-check, closeout-check, handoff-check, PR-body/task-packet checks | `lane_06_ci_github` |
