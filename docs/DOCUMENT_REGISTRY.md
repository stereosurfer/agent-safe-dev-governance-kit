...
in progress. The GitHub workflow and optional metadata-only catalog are
canonical under `scripts/asgk_lib/`; remaining `v3/` material is non-default
candidate source.
...
| `docs/EVOLUTION_MODEL.md` | canonical | document-driven ASGK self-evolution loop, reusable capability versus task-owned delivery state, evidence boundaries, independent handoff proof, and recovery rules | no | evolution-model changes, capability-discovery review, self-evolution review, or handoff-proof review | `lane_07_docs_handoff` |
...
| `scripts/asgk_lib/github_workflow.py` | script | sole GitHub workflow implementation: GET-only snapshot capture, issue-backed projection, draft-only handoff/closeout, supplied-snapshot search/trace with JSON or strict canonical YAML-subset shape-checked edges versus unverified YAML candidates, and common-envelope result boundary | no | GitHub workflow command, work projection, Kanban card draft or closeout-trace review | `lane_06_ci_github` |
| `scripts/asgk_lib/capability_evolution.py` | script | sole optional capability-catalog implementation: explicit-index metadata checks, one-level browse and bounded pointer selection with common-envelope proof limits; no content fetch, applicability decision or promotion | no | issue-scoped Lesson/method discovery, catalog CLI or regression review | `lane_06_ci_github` |
...
| `v3/capability_evolution.py`, `v3/test_capability_evolution.py` | script/fixture | thin catalog compatibility wrapper and required focused catalog regression tests invoked by root doctor | no | catalog source change, doctor/CI coverage audit, or preview compatibility review | `lane_06_ci_github` |
| `scripts/asgk_lib/scenario_registry.py` | script | sole executable owner of retained JSON owner commands, including target evidence and optional catalog, polarity, exact exits, results, finding-code multisets, human-gate states, proof boundaries, optional branch-specific checked/unchecked claims, task-packet alias parity, source-wrapper and catalog-wrapper parity | no | validator behavior, negative coverage, doctor, or CI scenario changes | `lane_06_ci_github` |
...
| `scripts/asgk.py` | script | ASGK CLI facade for canonical source validation, caller-supplied target evidence, GitHub workflow and optional catalog projections, common retained-JSON projections, and repository-governance checks; no mechanical command owns fit, depth, recommendation, or approval | no | local source validation, target evidence, workflow/catalog, status/closeout/handoff/PR checks, or policy-gate checks | `lane_06_ci_github` |
