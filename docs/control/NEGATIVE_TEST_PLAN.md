...
| `NEG-022-cloud-api-mcp-without-gate` | issue-scoped cloud/API/model/MCP capability opened without its applicable human gate | human_gated | AGENTS.md / Human-Gated Operations / PR review | `examples/negative/pr_body.external-call-no-gate.md` | Review-guidance case, not an executable scenario; missing issue authority is separately blocked by NEG-002. |
...
`SNAPSHOT_VERSION`) prevent known missing links, unsupported YAML, or absent supplied
