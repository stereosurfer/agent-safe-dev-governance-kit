...
| `NEG-006-pending-checks-merge` | PR says `merge_allowed` while checks are pending | blocked in body-coherence and strict modes | PR body policy gate | `examples/negative/policy_gate/pr_body.checks-pending.md` | Pending is valid only with a complete `merge_blocked` body. |
...
| `NEG-050-pending-human-gate-merge` | PR says `merge_allowed` while human review is pending | blocked in body-coherence and strict modes | PR body policy gate | `examples/negative/policy_gate/pr_body.human-gates-pending.md` | Human review cannot be self-certified or projected as complete. |
| `NEG-051-declared-blocked-pr-ready` | PR metadata, checks, review, paths, and body fields are clean but Merge Decision says `merge_blocked` | blocked | `check-pr` | `examples/negative/pr_status.merge-blocked-all-clean.json` | The durable result itself prevents merge eligibility. |
| `NEG-052-failed-checks-merge` | PR says `merge_allowed` while checks are false | blocked in body-coherence and strict modes | PR body policy gate | `examples/negative/policy_gate/pr_body.checks-false.md` | A known failed check cannot become a merge-ready claim. |
| `NEG-053-unsatisfied-human-gate-merge` | PR says `merge_allowed` while a known human gate is unsatisfied | blocked in body-coherence and strict modes | PR body policy gate | `examples/negative/policy_gate/pr_body.human-gates-false.md` | Human-gated work remains blocked until durable review is complete. |
