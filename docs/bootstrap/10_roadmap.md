...
active_milestone:
  ...
  latest_release_issue: "#321"
  latest_release_url: https://github.com/stereosurfer/agent-safe-dev-governance-kit/releases/tag/v1.7.3
...
completed_recent:
  ...
  source_local_boundary_hardening:
    result: "Issue #315 / PR #316 clarified target-install and Skill wording so ASGK source-repo validation/history semantics are not mistaken for generic target-repo completion rules; issue #317 published the source-only v1.7.2 patch release."
  pr_readiness_fallback_hardening:
    result: "Issue #319 / PR #320 clarified that `check-pr` is repository-local PR readiness evidence, not a universal target-repo command; issue #321 published the source-only v1.7.3 patch release."
...
likely_next_directions:
  ...
  future_v1_x_release:
    ...
    status: human_gated
    note: "Any release after v1.7.3 must start from a separate durable issue with exact tag, title, target commit, final validation, product-entry/handoff closeout plan, and rollback or revoke plan."
