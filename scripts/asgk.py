...
def cmd_negative(args: argparse.Namespace) -> int:
    ...
    if args.case == "compact-handoff":
        return run_expected_failures([
            [
                "python3", "scripts/asgk.py", "compact-handoff-check",
                *case,
            ]
            for case in COMPACT_HANDOFF_NEGATIVE_CASES
        ])
    if args.case == "compact-target-upgrade":
        return run_expected_failures([
            [
    ...
    if args.case == "all":
        ...
        compact_handoff = cmd_negative(argparse.Namespace(case="compact-handoff"))
        compact_target_upgrade = cmd_negative(argparse.Namespace(case="compact-target-upgrade"))
        return 1 if (
            ...
            or compact_task_packet or compact_pr_body or compact_handoff
            or compact_target_upgrade
        ) else 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("case", nargs="?", default="changed-paths", choices=["changed-paths", "textual", "policy-gate", "pr-status", "target-install", "release-state", "work-unit", "workspace-state", "compact-governance", "compact-issue-scope", "compact-scope-lock", "compact-pr-report", "compact-task-packet", "compact-pr-body", "compact-handoff", "compact-target-upgrade", "all"])
